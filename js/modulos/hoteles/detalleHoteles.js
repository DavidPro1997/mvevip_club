...
async function construirHabitaciones(info){
    ...
    const lista = armarStringHoteles(info.ocupantes)
...
function dividirHabitaciones(rooms){
    ...
    datos_por_habitacion.forEach(element => {
        lista = ""
        lista += obtenerRooms(1,element.rooms, element.id, codigoHotelGlobal, nombreHotelGlobal).lista
        lista += `<br><br><br><br>`
...
function escogerHotel(datos){
    ...
    if(resumenCompra.length==ocupantes.length){
        ...
        //ESTO HAY QUE QUITAR PARA PASAR AL CARRITO
